...
\item[\refqualclass{checker/regex/qual}{RegexNNGroups}]
  ...
  parameter for \code{@Regex}, and \<nonNullGroups> specifies the capturing
  groups that are definitely non-null (possibly empty) provided the regular
  expression matches the target string.  Note that \code{@Regex(1)} is
  exactly the same as \code{RegexNNGroups(groups = 1)}.
